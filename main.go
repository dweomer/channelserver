...
func run(c *cli.Context) error {
	...
	if len(SubKeys.Value()) != len(PathPrefix.Value()) {
		return errors.Errorf("keys-prefix lengths are not equal ", PathPrefix.Value(), SubKeys.Value(), ListenAddress)
	}
	fmt.Println(PathPrefix.Value(), SubKeys.Value())
	var configs []*config.Config
	for _, subkey := range SubKeys.Value() {
